...
function AdminHomeScreen() {
    ...
    const [isSellerApplicationTabActive, setSellerApplicationTabActive] = useState(false);
    ...
    const isAdmin = detailsData?.is_admin;
    const isManager = detailsData?.is_manager;
  ...
  return (
    ...
    <section className='w-full h-screen flex items-start justify-start overflow-hidden'>
...

            {
                isSidebarOpened ? (
                    <>
                        ...
                        <h3 className='text-dull mt-4 text-semibold'> {isAdmin ? "Admin" : isManager ? "Manager" : "Normal User"} </h3>
                        <div className='w-full flex items-center justify-start flex-col mt-8'>
                            <button onClick={() => {setDashboardTabActive(true); setSellerApplicationTabActive(false);}} className={`w-full py-4 text-dull transition-all duration-300 ${isDashboardTabActive ? "bg-less-primary text-primary font-semibold rounded-md" : ""}`}>Dashboard</button>
                            <button onClick={() => {setSellerApplicationTabActive(true); setDashboardTabActive(false);}} className={`w-full py-4 mt-1 text-dull transition-all duration-300 ${isSellerApplicationTabActive ? "bg-less-primary text-primary font-semibold rounded-md" : ""}`}>Seller Applications</button>
                        </div>
                    </>
                ) : (
        ...
        <section className='w-full h-full flex flex-col items-center justify-start relative' style={{background : "#f3faf6"}}>
            ...
            {
                isDashboardTabActive ? <DashboardScreen /> : ""
            }
            {
                isSellerApplicationTabActive ? <SellerApplicationsScreen /> : ""
            }
