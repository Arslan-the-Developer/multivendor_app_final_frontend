...
function AddStoreBasicDetailsScreen() {
    ...
    const { data: sellerData, error: sellerError, loading: sellerLoading } = useFetchData("authentication/check_seller", "get", null, 1);
    const { data: sellerStatusData, error: sellerStatusError, loading: sellerStatusLoading } = useFetchData("authentication/check_seller_status", "get", null, 1);

    const isBasicInfoAdded = sellerStatusData?.is_basic_info_added ?? false;
    const isIDInfoAdded = sellerStatusData?.is_id_info_added ?? false;

    // Determine user status
    const isAuthenticated = !authError || authError.status !== 403;
    const isSeller = sellerData?.is_seller;

    // console.log("Is Authenticated",isAuthenticated);
    // console.log("Is Seller",isSeller);
    // console.log("Is Approved Seller",sellerData?.is_approved_seller);
    ...
    async function handleDetailsSubmission(e) {
        ...
        try{
            ...
            const response = await axios({
                method: "post",
                url: "http://127.0.0.1:8000/authentication/seller_info_update",
